...
const TaskDetails = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    const proj = currentWorkspace.projects.find((item) => item.id === projectId);
    const tsk = proj?.tasks.find((item) => item.id === taskId);
                ...
                {comments.map((comment) => (
                  <div
                    ...
                    className={`sm:max-w-4/5 dark:bg-gradient-to-br dark:from-zinc-800 dark:to-zinc-900 border border-gray-300 dark:border-zinc-700 p-3 rounded-md ${
                      comment.user?.id === currentUser?.id ? "ml-auto" : "mr-auto"
                    }`}
                  >
