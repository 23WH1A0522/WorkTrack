...
const ProjectTasks = ({ tasks }) => {
  ...
  const assigneeList = useMemo(
    () => Array.from(new Set(tasks.map((task) => task.assignee?.name).filter(Boolean))),
    [tasks]
  );
        ...
        {["status", "type", "priority", "assignee"].map((name) => {
          const options = {
            ...
            assignee: [
              ...
              ...assigneeList.map((nameValue) => ({ label: nameValue, value: nameValue })),
            ],
          };
          return (
            <select
              ...
              <thead className="text-xs uppercase dark:bg-zinc-800/70 text-zinc-500 dark:text-zinc-400">
                <tr>
                  <th className="pl-2 pr-1">
                    <input
                      onChange={() =>
                        selectedTasks.length === tasks.length ? setSelectedTasks([]) : setSelectedTasks(tasks.map((task) => task.id))
                      }
                ...
                {filteredTasks.length > 0 ? (
                  filteredTasks.map((task) => {
                    ...
                    return (
                      ...
                      >
                        <td onClick={(event) => event.stopPropagation()} className="pl-2 pr-1">
                          <input
                            type="checkbox"
                            className="size-3 accent-zinc-600 dark:accent-zinc-500"
                            onChange={() =>
                              selectedTasks.includes(task.id)
                                ? setSelectedTasks(selectedTasks.filter((item) => item !== task.id))
                        ...
                        <td onClick={(event) => event.stopPropagation()} className="px-4 py-2">
                          <select
                            name="status"
                        ...
                        <td className="px-4 py-2">
                          <div className="flex items-center gap-2">
                            {task.assignee?.image ? (
                              ...
                              <div className="size-5 rounded-full bg-zinc-300 dark:bg-zinc-700" />
                            )}
                            {task.assignee?.name || "-"}
                          </div>
                        </td>
